...
public class DepreciationController extends HttpServlet {
    ...
    private void sendToDepreciation(HttpServletRequest request, HttpServletResponse response, boolean showMessage, String header, String message) throws IOException, ServletException
    {
        ...
        for(String d : new DepreciationHandler().getDepreciations())
        {
            deprecation += d + ",";
            deprecationInfo += new DepreciationHandler().getDepreciationInfo(d.split(";")[0])+"&&";
        }
        for(String d : new DepreciationHandler().getTimeIntervals())
        {
            depTimeIntervals += d + ",";
        }
        for(String a : new AssetHandler().getAssetsSelectorList())
        {
            assetSelectVal += a + ",";
        }
        
        request.setAttribute("depreciations", deprecation);
        request.setAttribute("depreciationInfos", deprecationInfo);
        request.setAttribute("depTimeIntervals", depTimeIntervals);
        request.setAttribute("assetSelector", assetSelectVal);
