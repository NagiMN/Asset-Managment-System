...
public class UserController extends HttpServlet {
    ...
    private void sendToUsers(HttpServletRequest request, HttpServletResponse response, boolean showMssg) throws IOException, ServletException {
        ...
        String jobCat = "";
        String clearances = "";
        for (String user : userHandler.getUsers()) {
            if (!user.equals(userHandler.getUsers().get(0))) {
                userList += ",";
            }

            userList += user;
        }
        for (String jC : userHandler.getJobCategories()) {
            if (!jC.equals(userHandler.getUsers().get(0))) {
                jobCat += ",";
            }

            jobCat += jC;
        }
        for (String clr : userHandler.getClearanceLevels()) {
            if (!clr.equals(userHandler.getUsers().get(0))) {
                clearances += ",";
            }
        ...
        request.setAttribute("clearances", clearances);
