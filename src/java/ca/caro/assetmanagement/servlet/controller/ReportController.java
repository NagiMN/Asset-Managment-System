...
public class ReportController extends HttpServlet {
    ...
    private void sendToReports(HttpServletRequest request, HttpServletResponse response, String message) throws IOException, ServletException {
        ...
        for (String user : assetHandler.getUsers()) {
            if (!user.equals(assetHandler.getUsers().get(0))) {
                userList += ",";
            }
            userList += user;
        }
        ...
        request.setAttribute("minDateInAssets", minDateInAssets);
        request.setAttribute("maxDateInAssets", maxDateInAssets);
